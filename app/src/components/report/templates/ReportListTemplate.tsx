      ...
      <DrawerContainer>
        <>
          ...
          {props.boxData.length > 0 ? (
            <Tabs>
              <Tabs.TabPane tab="ボックス表示" key="box">
                {props.boxData.map((dat) => {
                  return (
                    <BoxStyle key={dat.reportID}>
                      <ReportListBox {...dat} />
                    </BoxStyle>
                  );
                })}
              </Tabs.TabPane>
              <Tabs.TabPane tab="グラフ表示(β)" key="line">
                <ReportListChartRender data={props.chartData} />
              </Tabs.TabPane>
            </Tabs>
          ) : (
            <EmptyData>
              <Link to={routeBuilder.reportNewPath()}>
                <Button type="primary">レポート作成</Button>
              </Link>
            </EmptyData>
          )}
        </>
